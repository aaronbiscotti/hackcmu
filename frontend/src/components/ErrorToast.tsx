...
export default function ErrorToast({ message, isVisible, onClose }: ErrorToastProps) {
  ...
  return (
    <div className={`fixed top-6 right-6 z-50 transform transition-all duration-300 ease-out ${
      ...
      <div className="bg-red-500 text-snow p-4 shadow-2xl border border-red-400 max-w-sm">
        <div className="flex items-start gap-3">
          ...
          <button
            onClick={onClose}
            className="p-1 hover:bg-red-600 transition-colors duration-200 flex-shrink-0"
          >
            <XMarkIcon className="h-4 w-4 text-snow" />
          </button>
        </div>
        <div className="mt-3 bg-red-400 h-1 overflow-hidden">
          <div className="h-full bg-snow animate-shrink-width" />
        </div>
      </div>
    </div>
  );
